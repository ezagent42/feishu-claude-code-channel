...
7. `mkdir -p ~/.claude/channels/feishu/approved` then write
   ...
   file contents. The channel server polls this dir and sends "已配对".
...
- Sender IDs are Feishu `open_id` strings (e.g. `ou_xxxx`). Chat IDs are
  `chat_id` strings (e.g. `oc_yyyy`). Don't confuse the two.
